...
{
	s.chars().filter(|&s| s as char != ' ').collect::<String>()
}

fn remove_odd(a: &[i32]) -> Vec<i32>
{
	a.iter().filter(|i| **i % 2 == 0).map(|&x| x).collect::<Vec<i32>>()
}
...
{
	s.chars().filter(|&x| !is_vowel(x)).collect::<String>()
}

fn initialism(s: &str) -> String
{
	s.split_whitespace().map(|x| x.chars().next().unwrap()).collect::<String>().to_ascii_uppercase()
}

fn space_to_underscore(s: &str) -> String
{
	s.chars().map(|x| match x {' ' => '_', _ => x}).collect::<String>()
}
